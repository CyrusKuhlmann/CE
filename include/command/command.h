 ...
 *
 * Cancel Incoming causes newly scheduled commands to fail, keeping the current
 * command running until it ends uninterrupted. Cancel Running caused the
 * currently running command to yield to the newly scheduled command and end
 * interrupted.
 *
 */
enum class CommandCancelBehavior {
  /**
   * @brief Causes the newly scheduled command(s) to fail upon attempts to
   * schedule them while a command with CancelRunning is reserving the subsystem
   */
  CancelIncoming,
  /**
   * @brief Causes the currently running command to yield to the newly scheduled
   * command upon scheduling.
   */
  CancelRunning,
};
 ...
 * compartmentalize robot behaviors and provide the structure necessary to
 * ensure that multiple commands are attempting to access the same hardware at
 * the same time. Generally in projects you can use the classes such as \refitem
 * FunctionalCommand, or
 * \refitem RunCommand for simple Command behaviors, and you can override this
 * class to create more complex behaviors
 *
 */
class Command {
 public:
  /** @brief Called before every time the command is used. Users can override
   * this to create starting behaviors for custom commands
   *
   *  ```C
   *	void initialize() override {
   *		// Initialization tasks here, for example setting motor position
   * PID commands motor.move_absolute(5, 200);
   *	}
   *  ```
   */
  ...
  /**
   * @brief Execute is run every frame in the command scheduler(10ms/100Hz or
   * user selected) This can be useful for updating feedback controllers such as
   * PID or Pure Pursuit.
   *
   * ```C
  ...
  /**
   * @brief Stops the currently running command, this can be used to clean up
   * resources or finish up certain behaviors
   *
   * @param interrupted true if the command was stopped before isFinished
   * returned true, false otherwise
   */
  virtual void end(bool interrupted) {};

  /**
   * @brief This function returns the necessary subsystems needed to do run
   * command. The scheduler will work to free these requirements before running
   * the behavior to ensure no tasks overlap.
   *
   * @warning You must ensure all subsystems neccesary are captured by this
   * function or else multiple commands will overlap and run at the same time
   *
  ...
  /**
   * @brief Cancel this command if it is currently scheduled in the \refitem
   * CommandScheduler
   *
   * ```C
   * // Declare command(example)
  ...
  /**
   * @brief See if the command is currently scheduled in the \refitem
   * CommandScheduler
   *
   * ```C
   * // Declare command(example)
   ...
   * @return \refitem ParallelRaceGroup with this and \refitem WaitCommand of
   ...
   *
   * @warning Only use \refitem ProxyCommand where ABSOLUTELY necessary, it can
   * have unintended side effects. The side effects are listed in the class
   * documentation

...
/**
 * @brief Event loops store user-defined bindings to be run every frame. This is
 * used mostly to control the bindings necessary for \refitem Trigger
 */
class EventLoop {
 private:
 ...
 public:
   ...
   */
  explicit EventLoop(std::vector<std::function<void()>> bindings)
   ...
   */
  EventLoop(const std::initializer_list<std::function<void()>> bindings)
      : bindings(bindings) {}

  /**
   * @brief Poll is run every frame and runs each of the bindings. This is
   * generally run by the CommandScheduler
   */
  void poll() {

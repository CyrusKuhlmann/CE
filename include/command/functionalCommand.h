...
class FunctionalCommand : public Command {
 ...
 public:
  FunctionalCommand(std::function<void()> on_init,
                    std::function<void()> on_execute,
                    std::function<void(bool)> on_end,
                    std::function<bool()> is_finish,
                    const std::initializer_list<Subsystem*> requirements)
   ...
   */
  void initialize() override { onInit(); }

  /**
   * @brief Runs the user-defined execute function
   */
  void execute() override { onExecute(); }
   ...
   */
  bool isFinished() override { return isFinish(); }

  /**
   * @brief Runs the user-defined end function
   *
   * @param interrupted Passes on the interrupted parameter to the user-defined
   * function
   */
  void end(const bool interrupted) override { onEnd(interrupted); }
   ...
   */
  std::vector<Subsystem*> getRequirements() override { return requirements; }

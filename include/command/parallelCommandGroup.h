...
/**
 * @brief Runs multiple \refitem Command s at once, with the command ending once
 * all individual commands finsh.
 */
class ParallelCommandGroup : public Command {
 ...
 public:
  /**
   * @brief Create a new ParallelCommandGroup given a initializer list of
   * commands
   *
   * @warning No Commands in the parallel can require the same hardware! If this
   * happens the code will immediately abort
   *
   * @param commands Initializer list of \refitem Command s to run in this
   * parallel
   */
  ...
  bool isFinished() override {
    return std::ranges::all_of(commands | std::views::values,
                               [](const bool running) { return !running; });
  }

  /**
   * @brief Ends all commands with the ones that should be interrupted being
   * interrupted
   *

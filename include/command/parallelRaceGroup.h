...
class ParallelRaceGroup : public Command {
 ...
 public:
   ...
   * parallel
   */
  ParallelRaceGroup(const std::initializer_list<Command*> commands)
      : commands(commands) {
    isDone = false;
  ...
  /**
   * Runs all the active commands in the ParallelRaceGroup and check if any are
   * done
   */
   ...
   */
  bool isFinished() override { return this->isDone; }
